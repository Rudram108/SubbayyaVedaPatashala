		...
		*/
		function playSriSuktam(buttonNum)
			{
				//alert(anuNum);
		...
		if(getLearningMode() == 'com')
			var sourceAud = "audio/SriSuktam/PPCom.mp3";
		else if((buttonEle instanceof HTMLButtonElement) )
			sourceAud = "audio/SriSuktam/" + "ww_" + buttonNum +".mp3";
		else
			sourceAud = "audio/SriSuktam/" +getLearningMode()+"_" + buttonNum +".mp3";
		//alert(sourceAud);
...
inActiveButtonColor(curBut);					
		...
		*/
			function setLearningMode(mode){
				//alert('here');
			//document.getElementById("llCap").textContent() = "W2W";
		...
		if(mode == 'com'){
			try{
			source.src = "audio/SriSuktam/PPCom.mp3";
			if(!sounds.paused)
				{
					sounds.pause();								
				}
		...
		*/			
			function setTextMode(tMode){
				...
				var scr = "SriSuktam" + getLanguage() + tMode+".html";
			//alert(scr);
				var iframe = document.getElementById("anu");
			//alert("iframe");
